...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ 
  ...
  return (
    <>
      <div className={`bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group animate-scale-in border border-gray-200 ${!item.available ? 'opacity-60' : ''}`}>
        ...
        <div className="relative h-48 bg-gradient-to-br from-gray-50 to-gray-100">
          ...
          <div className={`absolute inset-0 flex items-center justify-center ${item.image ? 'hidden' : ''}`}>
            <div className="text-6xl opacity-20 text-gray-400">{categoryIcon}</div>
          </div>
          
          {/* Badges */}
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {item.isOnDiscount && item.discountPrice && (
              <div className="bg-rental-dark text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
                SALE
              </div>
            )}
            {item.popular && (
              <div className="bg-rental-purple text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
                ⭐ POPULAR
              </div>
            )}
          </div>
          ...
          {item.isOnDiscount && item.discountPrice && (
            <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm text-rental-dark text-xs font-bold px-2 py-1 rounded-full shadow-lg">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex-1">
              {item.isOnDiscount && item.discountPrice ? (
                ...
                <div className="text-2xl font-bold text-rental-dark">
                  ₱{item.basePrice.toFixed(2)}
                </div>
              )}
            ...
            <div className="flex-shrink-0">
              {!item.available ? (
                ...
                <button
                  ...
                  className="bg-rental-dark text-white px-6 py-2.5 rounded-xl hover:bg-rental-dark-hover transition-all duration-200 transform hover:scale-105 font-medium text-sm shadow-lg hover:shadow-xl"
                >
            ...
            <div className="p-6">
                            ...
                            <div className="flex items-center space-x-2">
                              {selectedAddOns.find(a => a.id === addOn.id) ? (
                                <div className="flex items-center space-x-2 bg-gray-100 rounded-xl p-1 border border-gray-200">
                                  ...
                                  >
                                    <Minus className="h-3 w-3 text-rental-dark" />
                                  </button>
                                  ...
                                  <button
                                    ...
                                    className="p-1.5 hover:bg-gray-200 rounded-lg transition-colors duration-200"
                                  >
                                    <Plus className="h-3 w-3 text-rental-dark" />
                                  </button>
                                </div>
                              ) : (
                                <button
                                  ...
                                  className="flex items-center space-x-1 px-4 py-2 bg-rental-dark text-white rounded-xl hover:bg-rental-dark-hover transition-all duration-200 text-sm font-medium shadow-lg"
                                >
              ...
              <div className="border-t border-gray-200 pt-4 mb-6">
                <div className="flex items-center justify-between text-2xl font-bold text-gray-900">
                  <span>Total:</span>
                  <span className="text-rental-dark">₱{calculatePrice().toFixed(2)}</span>
                </div>
              </div>

              <button
                onClick={handleCustomizedAddToCart}
                className="w-full bg-rental-dark text-white py-4 rounded-xl hover:bg-rental-dark-hover transition-all duration-200 font-semibold flex items-center justify-center space-x-2 shadow-lg hover:shadow-xl transform hover:scale-105"
              >
